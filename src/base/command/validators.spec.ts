...
describe('Base > Command > Validators', () => {
  describe('validateCommand()', () => {
    ...
    it('should throw CommandValidationError if builder name is too short.', () => {
      const expectedError = new CommandValidationError('Command / has an invalid name. It must be between 1 and 32 characters.');
      expect(() => validateCommand({ ...command, builder: { ...command.builder, name: '' } } as Command)).toThrow(expectedError);
    });

    it('should throw CommandValidationError if builder name is too long.', () => {
      const name = 'a'.repeat(35);
      const expectedError = new CommandValidationError(`Command /${name} has an invalid name. It must be between 1 and 32 characters.`);
      expect(() => validateCommand({ ...command, builder: { ...command.builder, name } } as Command)).toThrow(expectedError);
    });

    it('should throw CommandValidationError if builder name contains upper case letters.', () => {
      const name = 'COmmand';
      const expectedError = new CommandValidationError(`Command /${name} has an invalid name. Please use all lower-cased characters.`);
      expect(() => validateCommand({ ...command, builder: { ...command.builder, name } } as Command)).toThrow(expectedError);
    });

    it('should throw CommandValidationError if builder description is too short.', () => {
      ...
      expect(() => validateCommand({ ...command, builder: { ...command.builder, description: '' } } as Command)).toThrow(expectedError);
    });

    it('should throw CommandValidationError if bulder description is too long.', () => {
